...
document.addEventListener('DOMContentLoaded', () => {
  ...
  } catch (e) { console.warn('header scroll init failed', e); }
  ...
  try {
    ...
    if (prefersReducedMotion || !('IntersectionObserver' in window)) {
      ...
      // (covers edge cases where the observer never fires on some devices)
      setTimeout(() => {
        revealEls.forEach(el => el.classList.add('in-view'));
      }, 2500);
    }
  ...
  } catch (e) { console.warn('stat counter init failed', e); }
  ...
  /* ---------- The Path: scroll-traced spine with travelling marker ----------
     ...
     Desktop-only; disabled on narrow screens both in CSS and here. */
  try {
    if (window.innerWidth > 900) {
      ...
      function updateMarker() {
        ...
        let total = 0;
        try { total = spinePath.getTotalLength(); } catch (err) { return; }
        if (!total) return;
        const scrollTop = window.scrollY;
        const docHeight = document.documentElement.scrollHeight - window.innerHeight;
        const progress = docHeight > 0 ? Math.min(Math.max(scrollTop / docHeight, 0), 1) : 0;
        let point;
        try { point = spinePath.getPointAtLength(progress * total); } catch (err) { return; }
      ...
      function refreshSpine() {
        try { buildSpine(); updateMarker(); } catch (e) { console.warn('spine refresh failed', e); }
      }
  ...
  } catch (e) { console.warn('path spine init failed', e); }
  ...
  } catch (e) { console.warn('button pop init failed', e); }
